(function (window) {
	...
	function ActionProcessor() {
		...
		function send() {
			...
			if (!!window.navigator.sendBeacon) {
				...
				var abTestData = getABTestFromStorage(json.storeId);
				try {
					abTestData = JSON.parse(abTestData);
				} catch (e) {
					abTestData = null;
				}
				if (abTestData != null && (abTestData.group == null || abTestData.name == null)) {
					abTestData = null;
				}

				//limits the possible length of the test name and group to protect against unexpected data
				if (abTestData != null && (abTestData.group.length > 20 || abTestData.name.length > 50)) {
					abTestData = null;
				}

				json.storefrontAbTest = abTestData;
	...
	function getABTestFromStorage (storeId){
		try {
			return localStorage.getItem("PSecwid__" + storeId + "PSec__storefront__ab__test");
		} catch (e) {
			return null;
		}
	}
